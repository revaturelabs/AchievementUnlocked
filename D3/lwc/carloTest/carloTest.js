...
export default class CarloTest extends LightningElement {
    ...
    if (this.loadedRadar < 1) {
        ...
        try{
            ...
            //Make absolutely sure that the chart shows the relevant data
            ...
            //Clear out all existing radar charts
            let svgs = this.template.querySelectorAll("svg");
            svgs.forEach(function(item) {item.remove()});
    ...
    } else if (this.loadedRadar <= 1) {console.log ("loadedRadar is " + this.loadedRadar)}
    ...
    radarChart(id, data, options) {
        //Making sure the data is accurate AGAIN
        this.selectedCohort = this.template.querySelector('.cohortSelector').value;
        this.selectedExam = this.template.querySelector('.examSelector').value;
        ...
        //Clear out all other radar charts
        let svgs = this.template.querySelectorAll("svg");
        svgs.forEach(function(item) {item.remove()});
